...
test('ContentSecurityPolicyError', () => {
  ...
  const sourceFile =
    'http://localhost:3000/packages/extension-host-worker-tests/fixtures/sample.error-import-data-url-csp-violation/main.js'
  const lineNumber = 1
  const columnNumber = 11
  const error = new ContentSecurityPolicyError(
    violatedDirective,
    sourceFile,
    lineNumber,
    columnNumber,
  ...
  expect(error.message).toBe(
    'Content Security Policy Violation: script-src-elem',
  )
